...
int main(int argc, char* argv[]){
    ...
    // Dust particles
    double a_p, e_p, i_p, Omega_p, omega_p;  /* particle semimajor axis and eccentricity */
    double E_p = M_PI;
    unsigned int N_a = 300;
    unsigned int N_e = 30;
    unsigned int N_i = 1;
	...
	a_p = generate_random_double(500.0, 6500.0);
        for ( unsigned int j = 0; j<N_e; j++ ) {
            e_p = generate_random_double(0.0, 1.0);
            for ( unsigned int k = 0; k<N_i; k++ ) {
                i_p = (double)k*M_PI/3.0/(double)N_i;
                for ( unsigned int l = 0; l<N_Omega; l++ ) {
                    Omega_p = (double)l*2.0*M_PI/(double)N_Omega;
                    for ( unsigned int m = 0; m<N_omega; m++ ) {
                        omega_p = (double)m*2.0*M_PI/(double)N_omega;
                        
                        struct reb_particle p = {0};
                        p.m = 0.0;  // test particles
                        p.r = r_dust;
                        
                        double sini = sin(i_p);
                        double cosi = cos(i_p);
                        double sinO = sin(Omega_p);
                        double cosO = cos(Omega_p);
                        double sinw = sin(omega_p);
                        double cosw = cos(omega_p);
                        double PVector[3] = { cosO*cosw-sinO*sinw*cosi, sinO*cosw+cosO*sinw*cosi, sinw*sini };
                        double QVector[3] = { -cosO*sinw-sinO*cosw*cosi, -sinO*sinw+cosO*cosw*cosi, cosw*sini };
                        double pos[3], vel[3];
                        pos[0] = a_p * ( ( cos(E_p) - e_p ) * PVector[0] + sqrt(1.0 - e_p*e_p)* sin(E_p) * QVector[0] );
                        pos[1] = a_p * ( ( cos(E_p) - e_p ) * PVector[1] + sqrt(1.0 - e_p*e_p)* sin(E_p) * QVector[1] );
                        pos[2] = a_p * ( ( cos(E_p) - e_p ) * PVector[2] + sqrt(1.0 - e_p*e_p)* sin(E_p) * QVector[2] );
                        sini = sqrt( pos[0]*pos[0] + pos[1]*pos[1] + pos[2]*pos[2] ); // used as a temp
                        vel[0] = sqrt( r->G * mass_system * a_p )/sini * ( -sin(E_p)*PVector[0] + sqrt(1.0 - e_p*e_p)*cos(E_p)*QVector[0] );
                        vel[1] = sqrt( r->G * mass_system * a_p )/sini * ( -sin(E_p)*PVector[1] + sqrt(1.0 - e_p*e_p)*cos(E_p)*QVector[1] );
                        vel[2] = sqrt( r->G * mass_system * a_p )/sini * ( -sin(E_p)*PVector[2] + sqrt(1.0 - e_p*e_p)*cos(E_p)*QVector[2] );
                            
                        p.x = pos[0];
                        p.y = pos[1];
                        p.z = pos[2];
                        p.vx = vel[0] + T11 * v_Didy_Bary_x + T12 * v_Didy_Bary_y + T13 * v_Didy_Bary_z;
                        p.vy = vel[1] + T21 * v_Didy_Bary_x + T22 * v_Didy_Bary_y + T23 * v_Didy_Bary_z;
                        p.vz = vel[2] + T31 * v_Didy_Bary_x + T32 * v_Didy_Bary_y + T33 * v_Didy_Bary_z;
