...
void double_perpendicular_velocity(struct reb_particle sun, struct reb_particle* p);
...
/**
 * Doubles the velocity of a particle in the direction perpendicular 
 * to the solar radiation pressure.
 */
void double_perpendicular_velocity(struct reb_particle sun, struct reb_particle* p) {
	// 1. Calculate the SRP direction vector (Sun -> Particle)
	double dx = p->x - sun.x;
	double dy = p->y - sun.y;
	double dz = p->z - sun.z;
	...
	// Safety check to avoid division by zero if particle is at the Sun's center
	if (r_mag > 1e-9) {
		// Normalize the SRP direction vector (unit vector n)
		double nx = dx / r_mag;
		double ny = dy / r_mag;
		double nz = dz / r_mag;

		// 2. Project velocity onto the SRP unit vector (Parallel Component)
		// v_parallel = (v · n) * n
		double v_dot_n = (p->vx * nx) + (p->vy * ny) + (p->vz * nz);
		
		double v_para_x = v_dot_n * nx;
		double v_para_y = v_dot_n * ny;
		double v_para_z = v_dot_n * nz;

		// 3. Find the Perpendicular Component
		// v_perp = v_total - v_parallel
		double v_perp_x = p->vx - v_para_x;
		double v_perp_y = p->vy - v_para_y;
		double v_perp_z = p->vz - v_para_z;

		// 4. Double the perpendicular component
		// New Velocity = v_parallel + 2 * v_perp
		// Equivalently: New Velocity = Old Velocity + v_perp
		p->vx += v_perp_x;
		p->vy += v_perp_y;
		p->vz += v_perp_z;
	}
