...
# data path
data_rootdir = "/home/linfel/linfel_data/test_data"
...
output_dir = "/home/linfel/linfel_data/test_data/testplot_0422"
...
print(f"printing T0+{day} day, {new_time}")
...
for i in range(N_datasets):
    Np_seq, time, r_dust, data_p = read_particle_frames(filenames[i])
    
    # set group label
    group_labels[i+1] = [f"{r_dust:.4f}", '#{:02x}{:02x}{:02x}'.format(
        int(colors[i][0] * 255),
        int(colors[i][1] * 255),
        int(colors[i][2] * 255))]
