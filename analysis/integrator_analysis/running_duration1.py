...
def get_runtime(parent_dir = "."):
    results = {}
    pattern = re.compile(r"(\d+\.\d+)\s*$") # search pattern in the dt_history.csv file
    ...
    for folder in sorted(os.listdir(parent_dir)):
        if folder.startswith("run_") and os.path.isdir(os.path.join(parent_dir, folder)):
            ...
            file2 = os.path.join(run_path, "dt_history.csv")
            file3 = os.path.join(run_path, "particles.txt")
            ...
            if os.path.exists(file1) and os.path.exists(file2) and os.path.exists(file3):
                ...
                # --- Extract t/tmax info from the system log file ---
                progress = None
                logfile = file2
                try:
                    output = subprocess.check_output(["tail", "-n", "2", logfile], text=True)
